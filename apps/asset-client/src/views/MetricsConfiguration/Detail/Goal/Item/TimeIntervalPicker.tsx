...
import { TimeType } from 'store/slices/metricsConfiguration/detail/type';
import { timeTypes } from 'store/slices/metricsConfiguration/detail/data';
        ...
        {timeTypes.map((type) => (
          <CatMenuItem key={type} value={type}>
            <CatTypography className="opacity-6" variant="caption">
              {t(`metrics_configuration.detail.goals.time_types.${type}`)}
            </CatTypography>
          </CatMenuItem>
        ))}
      </CatSelect>
